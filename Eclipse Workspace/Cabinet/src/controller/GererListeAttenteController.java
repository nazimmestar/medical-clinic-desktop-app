...
public class GererListeAttenteController implements Initializable {
    ...
    public ResultSet result ;
	    ...
	    void ajouter(MouseEvent event) {
	    	...
	    	if(!nom.equals("")&&!prenom.equals("")&&!tel.equals("")&&!heure.equals("")) {
	    		try {
				    ...
				    txt_tel.setText("");
				    txt_heure.setText("");
				    Alert alert = new Alert(AlertType.CONFIRMATION,"ajouté a la liste avec succès!",javafx.scene.control.ButtonType.OK);
				    alert.showAndWait();
				    showListe();
	    ...
	    void supprimer(MouseEvent event) {
	        Liste selectedListe = table_liste.getSelectionModel().getSelectedItem();
	        String sql="delete from liste where idL ='"+selectedListe.getId()+"'" ;
	    	try {
				st=cnx.prepareStatement(sql);
				st.executeUpdate();
				showListe();
			    ...
			    txt_tel.setText("");
			    txt_heure.setText("");
			    Alert alert = new Alert(AlertType.CONFIRMATION,"supprimé avec succès!",javafx.scene.control.ButtonType.OK);
			    alert.showAndWait();  
			} catch (SQLException e) {
				e.printStackTrace();
		    }
