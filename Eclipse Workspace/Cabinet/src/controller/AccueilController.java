...
public class AccueilController implements Initializable {
    ...
    void finish(MouseEvent event) {
    	...
    	if(b!=null) {
    		txt_search.setValue(null); // Clear the search field
	        table_rdv.getSelectionModel().clearSelection(); // Clear any row selection
	    	data.clear();
	    	showAgenda();
    ...
    public int getNumberOfPatients() {
        ...
        try {
            ...
            result = st.executeQuery();
            if (result.next()) {
                numberOfPatients = result.getInt(1);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public int getNumberOfliste() {
        int numberOfPatients = 0;
        String sql = "SELECT COUNT(*) FROM liste";
        try {
            st = cnx.prepareStatement(sql);
            result = st.executeQuery();
            if (result.next()) {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		nbr_p.setText(Integer.toString(numberOfPatients));
		int numberOfliste = getNumberOfliste();
		nbr_l.setText(Integer.toString(numberOfliste));
	}
