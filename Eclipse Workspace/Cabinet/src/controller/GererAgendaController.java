...
public class GererAgendaController implements Initializable {
    ...
    public ResultSet result ;
	    ...
	    void modifierA(MouseEvent event) {
	    	...
	    	String sql="update agenda set nom=?,prenom=?,numTel=?,dateR=?,heure=? where idA ='"+selectedRDV.getId()+"'" ;
	    	try {
				st=cnx.prepareStatement(sql);
				st.setString(1, nom);
				java.util.Date date =java.util.Date.from(DatePicker.getValue().atStartOfDay(ZoneId.systemDefault()).toInstant());
			    ...
			    txt_heure.setText("");
			    DatePicker.setValue(null);
			    Alert alert = new Alert(AlertType.CONFIRMATION,"rendez_vous modifié avec succès!",javafx.scene.control.ButtonType.OK);
			    alert.showAndWait();
			    showAgenda();
			} catch (SQLException e) {
				e.printStackTrace();
	    ...
	    void searchA(MouseEvent event) {
	        ...
	        if (!filteredList.isEmpty()) {
	            ...
	            Alert alert = new Alert(AlertType.ERROR, "Aucun Date trouvé avec  " + searchDate, ButtonType.OK);
	            alert.showAndWait();
	        }
	    }
	    	else{
	    ...
	    void finish(MouseEvent event) {
	    	...
	    	LocalDate b = txt_search.getValue();
	    	if(b==null) {
	    		clearFields();
	    	}
	    	DatePicker.setValue(null); // Clear the search field
	        table_agenda.getSelectionModel().clearSelection(); // Clear any row selection
	    	data.clear();
	    	showAgenda();
	        clearFields();
	    ...
	    void supprimerA(MouseEvent event) {
	        ...
	        Agenda selectedRDV = table_agenda.getSelectionModel().getSelectedItem();
	    	String sql="delete from agenda where idA ='"+selectedRDV.getId()+"'" ;
	    	try {
				st=cnx.prepareStatement(sql);
				st.executeUpdate();
				showAgenda();
